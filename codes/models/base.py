...
class BaseModel(nn.Module):
            ...
            for _input in data_loader:
                ...
                if result["conf"][0] >= self.confidence:
                    sample = {
                        ...
                        "log_features": _input["log_features"]
                    }
                    data.append(sample)
    ...
    def collate_fn(self,batchs):
        ...
        log_features.append(batch['log_features'])
        kpi_features = []
        kpi_features_torchTensor = []
        for kpi_feature in batch['kpi_features']:
            kpi_features.append(np.expand_dims(kpi_feature, axis=0))
        for batch_i in range(1, len(batchs)):
            labels.append(batchs[batch_i]['label'])
            idx.append(batchs[batch_i]['idx'])
            log_features.append(batchs[batch_i]['log_features'])
            for kpi_i in range(len(batchs[batch_i]['kpi_features'])):
                kpi_features[kpi_i] = np.concatenate(
                    [kpi_features[kpi_i], np.expand_dims(batchs[batch_i]['kpi_features'][kpi_i], axis=0)], axis=0)

        for kpi_feature in kpi_features:
            kpi_features_torchTensor.append(torch.tensor(kpi_feature, dtype=torch.float32))
        # 使用 DGL 提供的 batch 方法合并图
        batched_graph = dgl.batch(log_features)
        # 将标签合并为张量
        labels = torch.tensor(labels, dtype=torch.long)
        idx = torch.tensor(idx, dtype=torch.long)
        # kpi_features = torch.tensor(kpi_features, dtype=torch.float32)
        # return batched_graph, labels
        return {"idx": idx, "label": labels, "kpi_features": kpi_features_torchTensor, "log_features": batched_graph}
    ...
    def fit(self, train_loader, unlabel_loader, test_loader):
        ...
        pseudo_loader = DataLoader(pseudo_data, batch_size=self.batch_size, shuffle=True,collate_fn=self.collate_fn)
