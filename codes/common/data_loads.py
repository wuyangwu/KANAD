...
def load_sessions(data_dir): #both log and kpi
    logging.info("Load from {}".format(data_dir))# train.pkl 数据是10227条 测试是 19239条
...
class Process():
    def __init__(self, var_nums, labeled_train, unlabel_train, test_chunks, supervised=False, **kwargs):
        ...
        labeled_train = self.ext.transformGraph(labeled_train)
        # test_chunks = self.ext.transform(test_chunks, datatype="test")
        test_chunks = self.ext.transformGraph(test_chunks, datatype="test")
        ...
        if not supervised:
            ...
            unlabel_train = self.ext.transformGraph(unlabel_train, datatype="unlabel train")
